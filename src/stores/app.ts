...
const useAppStore = defineStore('app', () => {
  ...
  const storedHomePingNetworkMode = useStorageAsync<HomePingNetworkMode | null>('homePingNetworkMode', null, localStorage)
  const homePingTaskSelections = useStorageAsync<Partial<Record<KnownPingNetworkFamily, string>>>('homePingTaskSelections', {}, localStorage)
  ...
  function isValidHomePingNetworkMode(value: unknown): value is HomePingNetworkMode {
    return value === 'auto' || value === 'all' || (KNOWN_PING_NETWORK_FAMILIES as readonly string[]).includes(String(value))
  }
  ...
  const homePingNetworkMode = computed<HomePingNetworkMode>({
    get: () => {
      if (isValidHomePingNetworkMode(storedHomePingNetworkMode.value))
        return storedHomePingNetworkMode.value
      return defaultHomePingNetworkMode.value
    },
    set: (value) => {
      storedHomePingNetworkMode.value = normalizePingNetworkMode(value, 'auto')
    },
  })

  const homePingPreferredTaskKeywords = computed<Partial<Record<KnownPingNetworkFamily, string[]>>>(() => {
    return parsePingTaskPreferenceText(themeSettings.value.homePingPreferredTasks)
  })

  function setHomePingTaskSelection(family: KnownPingNetworkFamily, taskId: string): void {
    const nextSelections = { ...homePingTaskSelections.value }
    if (taskId)
      nextSelections[family] = taskId
    else
      delete nextSelections[family]
    homePingTaskSelections.value = nextSelections
  }
  ...
  watch(publicSettings, (settings) => {
    ...
    if (settings && !isValidHomePingNetworkMode(storedHomePingNetworkMode.value))
      storedHomePingNetworkMode.value = defaultHomePingNetworkMode.value
  }, { immediate: true })
  ...
  watch(storedHomePingNetworkMode, (mode) => {
    if (mode !== null && !isValidHomePingNetworkMode(mode))
      storedHomePingNetworkMode.value = defaultHomePingNetworkMode.value
  }, { immediate: true })
